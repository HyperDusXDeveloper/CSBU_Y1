...
def registersubmit() : 
    ...
    if name.get() == "" :
        ...
        messagebox.showwarning("Admin","Password and Confirm Password")
        password.delete(0,END)
        cfpassword.delete(0,END)
        password.focus_force()
    else :
        sql = "select * from login where user=? collate nocase"
        ...
        if result :
            ...
            result = cursor.fetchall()
            if result : 
                messagebox.showwarning("Admin" ,"You have registered \n Plesaeback to login")
                ...
                conn.commit()
                messagebox.showinfo("Admin" , "Register seccessfully")
                backtologin()
