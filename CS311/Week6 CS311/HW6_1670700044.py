...
def leftframe(left) :
    #Show Button .grid
    Button(left,text='Product Menu',image=icon1,compound='top',command=productmenu).grid(row=0 , column=0,sticky='news')
    Button(left,text='Checkout',image=icon2,compound='top',command=checkoutclick).grid(row=1 , column=0,sticky='news')
    Button(left,text='Close Program',image=icon5,compound='left',command=exit).grid(row=2 , column=0,sticky='news')

def productmenu() :
    checkoutframe.grid_forget()
    pmenu.grid_columnconfigure((0,1,2),weight=1)
    pmenu.grid_rowconfigure((0,1,2),weight=1)
    pmenu.grid(row=0,column=1,sticky='news')
