...
def keyboardcrick() :
    global keyboardframe,keyboardresult
    ...
    # Label(keyboardframe,text='hyperx',image=HYPERXALLOYORIGINS,bg='#ffffff',fg='#000000',font="Helvetica 40 ",compound='top').grid(row=1,sticky=NW,pady=10)
    cursor.execute('SELECT productimagename , productname , price FROM keyboardproduct')
    keyboardresult = cursor.fetchall()
    keyboardimage = [ASUSROGAZOTH,EGATYPECMK9,HYPERXALLOYORIGINS,KEYCHRONK10MAXSILENT,LOGITECHGG515,REDRAGONPOLLUXPRO]
    for idx, k in enumerate(keyboardresult):
        imagename = k[0]
        productname = k[1]
        price = k[2]

    img = keyboardimage(imagename, None)

    if img:
        Label(keyboardframe,text=f"{productname}\nPrice: {price}",image=img,compound='top',bg='#ffffff',fg='#000000',font="Helvetica 16").grid(row=idx, column=0, sticky=NW, pady=10)
    else:
        Label(keyboardframe,text=f"{productname}\nPrice: {price}\n[No Image]",bg='#ffffff',fg='#000000',font="Helvetica 16").grid(row=idx, column=0, sticky=NW, pady=10)

    # keyboardimage = [ASUSROGAZOTH,EGATYPECMK9,HYPERXALLOYORIGINS,KEYCHRONK10MAXSILENT,LOGITECHGG515,REDRAGONPOLLUXPRO]
    # cursor.execute('SELECT productimagename , productname , price FROM keyboardproduct')
    # keyboardresult = cursor.fetchall()
    # for k in keyboardresult:
    #     print(str(k[0]) + " " + str(k[1]) + " " + str(k[2]) + " " )
    #     Label(keyboardframe,text=str(k[1]),image=keyboardimage[k],bg='#ffffff',fg='#000000',font="Helvetica 40 ",compound='top').grid(row=k,sticky=NW,pady=10)
    
    
    
    # keyboardimage = []
    # for k in keyboardresult:
    #     keyboardimage.append(k[0])
    #     print("img"+str(keyboardimage))
    #     print(k[0] + " " + str(k[1]) + " " + str(k[2]) + " " )
    #     Label(keyboardframe,text=str(k[1]),image=keyboardimage[k],bg='#ffffff',fg='#000000',font="Helvetica 40 ",compound='top').grid(row=k,sticky=NW,pady=10)
...
ASUSROGAZOTH = PhotoImage(file='images/ASUSROGAZOTH.png').subsample(2,2)
EGATYPECMK9 = PhotoImage(file='images/EGATYPECMK9.png').subsample(2,2)
HYPERXALLOYORIGINS = PhotoImage(file='images/HYPERXALLOYORIGINS.png').subsample(2,2)
KEYCHRONK10MAXSILENT = PhotoImage(file='images/KEYCHRONK10MAXSILENT.png').subsample(2,2)
LOGITECHGG515 = PhotoImage(file='images/LOGITECHGG515.png').subsample(2,2)
REDRAGONPOLLUXPRO = PhotoImage(file='images/REDRAGONPOLLUXPRO.png').subsample(2,2)
introshop()
root.mainloop()
cursor.close()
conn.close()
