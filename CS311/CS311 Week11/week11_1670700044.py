...
def updateclick() :
    ...
    Label(top,text="Update Window",bg='#493628',font=('Garamond',32,'bold'),fg='white').grid(row=1)
...
def updatepage(result) :
    ...
    nameupdate.grid(row=0,column=1,sticky=W)
    userupdate = Entry(right,textvariable=userinfo,fg='blue',state=DISABLED)
    userupdate.grid(row=1,column=1,sticky=W)
    pwdupdate = Entry(right,textvariable=pwdinfo,fg='blue',show='*')
    pwdupdate.grid(row=2,column=1,sticky=W)
    cfupdate = Entry(right,textvariable=cfinfo,fg='blue',show='*')
    cfupdate.grid(row=3,column=1,sticky=W)
    Button(right,text="Update",command=updatesubmit).grid(row=4 , column=1 ,ipady=10 , pady=10 ,sticky=W ,padx=10)
    Button(right,text="Go to Login",command=backtologin).grid(row=4 , column=0 ,ipady=10 , pady=10 ,sticky=E ,padx=10)
...
def updatesubmit() :
    if nameinfo.get() == "" :
        ...
        messagebox.showwarning("Admin", "Please enter confirm password not match")
        pwdupdate.delete(0,END)
        cfupdate.delete(0,END)
        pwdupdate.focus_force()
    else :
        ...
        conn.commit()
        messagebox.showinfo("Admin","Update secussfully")
        # print('Update Submit')
